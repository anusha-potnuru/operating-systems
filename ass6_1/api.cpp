...
void init()
{
	...
	sb->d = NULL;
	sb->bit_vector = (bool*)malloc(sizeof(bool)*no_of_blocks);
	sb->bit_vector[0]=-1;
	sb->bit_vector[1]=-1;
}
...
int my_write(int fd, char *buf,int size, int flag)
{
	...
	while(size>=block_size)
	{
		...
		// cout<<strlen(file_system[bn].b_data)<<endl;
		buf = buf+block_size;
		// cout<<"done"<<endl;
		sb->bit_vector[bn]=1;
		prev1 = bn;
...
void my_cat(int fd)
{
	...
	{
		// cout<<"block number "<<bn<<endl;
		// printf("%s",file_system[bn].b_data);
