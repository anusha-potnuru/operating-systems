...
for i in range(30):
	p = subprocess.Popen(['.//schedule'],stdin=subprocess.PIPE,stdout=subprocess.PIPE)
	if(i//10==0):
...
colors = ["green", "blue", "red", "cyan", "black"]
...
for i in range(5):
	...
	if(i==4):
		x = arr5
	plt.scatter(y, x, c=colors[i], alpha=0.5, marker='.', label= labels[i])
